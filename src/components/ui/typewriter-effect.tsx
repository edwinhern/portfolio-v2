          ...
          return (
            <div key={`word-${idx}`} className="inline-block">
              {word.text.map((char, index) => (
                <motion.span initial={{}} key={`char-${index}`} className={cn(`hidden`, word.className)}>
                  {char}
                </motion.span>
              ))}
              &nbsp;
      ...
      <div>
        {wordsArray.map((word, idx) => {
          return (
            <div key={`word-${idx}`} className="inline-block">
              {word.text.map((char, index) => (
                <Balancer key={`char-${index}`} className={cn(word.className)}>
                  {char}
                </Balancer>
              ))}
              &nbsp;
  ...
  return (
    <div className={cn('my-6 flex space-x-1', className)}>
      ...
      <motion.span
        ...
        className={cn('block h-4 w-[4px] rounded-sm bg-foreground sm:h-6 xl:h-12', cursorClassName)}
      ></motion.span>
    </div>
  );
};
